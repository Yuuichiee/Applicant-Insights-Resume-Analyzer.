...
import spacy
import os
# Force PyResParser to use correct config path
config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "config.cfg")
os.environ["PYRESPARSER_CONFIG"] = config_path
...
st.set_page_config(
    page_title="Applicant Insights Resume Analyzer",
    #page_icon='./Logo/SRA_Logo.ico',
)


def run():
    st.title("Applicant Insights Resume Analyzer")
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'Shivam' and ad_password == '123':
                st.success("Welcome Shivam")
                # Display Data
